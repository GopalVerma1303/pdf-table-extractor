...
class PDFTableExtractor:
    ...
    def extract_tables_from_pdf(self, pdf_path, output_excel_path=None):
        """
        Extract tables from a PDF file and save to Excel

        Args:
            pdf_path: Path to the PDF file
            output_excel_path: Path to save the Excel file (optional)
        ...
        if output_excel_path is None:
            ...
            output_excel_path = f"{base_name}_tables.xlsx"

        logger.info(f"Processing PDF: {pdf_path}")
        logger.info(f"Output will be saved to: {output_excel_path}")
        ...
        # Save all tables to Excel if any were found
        if all_tables:
            self._save_tables_to_excel(all_tables, output_excel_path)
            logger.info(
                f"Successfully extracted {len(all_tables)} tables to {output_excel_path}"
            )
        else:
    ...
    def _save_tables_to_excel(self, tables, output_path):
...
def main():
    ...
    extractor.extract_tables_from_pdf(args.pdf_path, args.output)
